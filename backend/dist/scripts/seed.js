...
async function run() {
    ...
    const experiences = [
        ...
        },
        {
            ...
            price_per_person: 75,
            image_url: 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?q=80&w=1600&auto=format&fit=crop',
            rating: 4.7,
            reviews_count: 540,
            slots: [
    ...
    for (const exp of experiences) {
        const result = await pool.query(`INSERT INTO experiences (title, location, description, price_per_person, image_url, rating, reviews_count)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [exp.title, exp.location, exp.description, exp.price_per_person, exp.image_url, exp.rating, exp.reviews_count]);
        const experienceId = result.rows[0].id;
        for (const slot of exp.slots) {
            await pool.query(`INSERT INTO slots (experience_id, slot_id, date, timeslot, capacity)
         VALUES ($1, $2, $3, $4, $5)`, [experienceId, slot.slot_id, slot.date, slot.timeslot, slot.capacity]);
        }
    }
